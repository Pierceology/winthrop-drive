...
export class AmbientLife{
 constructor(scene,network,heightAt){Object.assign(this,{scene,network,heightAt});this.group=new THREE.Group();scene.add(this.group);this.agents=[];this.clock=0;this.nodes=new Map();this.segments=network.segments.filter(s=>s.width>=6&&s.length>8&&s.direction!==null);const key=p=>p.map(v=>Math.round(v)).join(',');this.key=key;
 for(const s of network.segments){if(s.width<6||s.direction===null)continue;for(const reverse of(s.direction===1?[false]:s.direction===-1?[true]:[false,true])){const a=reverse?s.b:s.a,b=reverse?s.a:s.b,edge={...s,a,b,dx:b[0]-a[0],dz:b[1]-a[1]};const k=key(a);if(!this.nodes.has(k))this.nodes.set(k,[]);this.nodes.get(k).push(edge);}}
 ...
 spawn(agent,focus){const candidates=this.segments.filter(s=>Math.hypot((s.a[0]+s.b[0])/2-focus.x,(s.a[1]+s.b[1])/2-focus.z)<450&&(!agent.pedestrian||/SHIRLEY|BEACH|WINTHROP|HAGMAN|WOOD/i.test(s.name)));if(!candidates.length){agent.model.visible=false;agent.edge=null;return;}agent.edge=candidates[(agent.i*17+Math.floor(this.clock/30)*7)%candidates.length];if(agent.edge.direction===-1||(agent.edge.direction===0&&agent.i%2)){const s=agent.edge;agent.edge={...s,a:s.b,b:s.a,dx:-s.dx,dz:-s.dz};}agent.t=.15+(agent.i%7)*.1;agent.model.visible=true;}
 ...
 for(const a of this.agents){if(!a.edge||Math.hypot(a.model.position.x-focus.x,a.model.position.z-focus.z)>800)this.spawn(a,focus);if(!a.edge)continue;const p=this.point(a);let speed=a.speed;
  if(!a.pedestrian){const others=this.agents.filter(b=>b!==a&&!b.pedestrian&&b.edge&&b.model.visible).map(b=>b.model.position);if(player?.active)others.push({x:player.state.x,z:player.state.z});for(const b of others){const dx=b.x-p.x,dz=b.z-p.z,ahead=(dx*a.edge.dx+dz*a.edge.dz)/a.edge.length,side=Math.abs(dx*a.edge.dz-dz*a.edge.dx)/a.edge.length;if(ahead>0&&ahead<10&&side<2)speed=Math.min(speed,Math.max(0,ahead-5));}
   if(this.lights){const toEnd=(1-a.t)*a.edge.length;if(toEnd<7){const l=this.lights.lightAhead(a.edge.b[0],a.edge.b[1],a.edge.dx/a.edge.length,a.edge.dz/a.edge.length,this.clock,2);if(l&&l.state!=='green'&&!(l.state==='yellow'&&toEnd<3))speed=0;}}}
  a.t+=dt*speed/a.edge.length;if(a.t>1){const choices=(this.nodes.get(this.key(a.edge.b))||[]).filter(s=>(s.dx*a.edge.dx+s.dz*a.edge.dz)/(s.length*a.edge.length)>-.6);if(choices.length){a.edge=choices[(a.i+Math.floor(this.clock/12))%choices.length];a.t=0;}else{a.edge=null;a.model.visible=false;continue;}}
