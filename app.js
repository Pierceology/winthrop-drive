...
async function init(){
 ...
 if(!lowMemory&&!FACTORY)try{const [idx,buf]=await Promise.all([get(DATA+'roofs-index.json'),fetch(DATA+'roofs.bin').then(r=>r.arrayBuffer())]);const dv=new DataView(buf);let applied=0;const centers=new Map(world.buildings.map(b=>[b.id,b.center]));
 ...
 groundWorld=await buildGroundWorld(foundationData,roofTiles,origin,asphaltMaterial(),FACTORY?null:ortho.material);   /* a factory town's ground is its NAIP tiles; Winthrop's is the streamed ortho */
...
window.__drive=driving;window.__controls=controls;window.__camera=camera;
 ...
 try{canopyData=await get(DATA+'tree-survey.json');canopyGroup=treeSurvey(canopyData,surfaceAt);scene.add(canopyGroup);
