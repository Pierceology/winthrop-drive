...
async function init(){
 ...
 const canopyAreas=await get('./data/canopy-scenery.json');sceneryTrees=await treeScenery(canopyData,surfaceAt,driving.state.network,canopyAreas);scene.add(sceneryTrees);$('treeSummary').textContent=sceneryTrees.userData.count.toLocaleString()+' tree models: '+sceneryTrees.userData.screened+' screened crown locations and '+sceneryTrees.userData.areaCount.toLocaleString()+' placements within mapped canopy areas. Individual trunks, heights and species estimated.';
