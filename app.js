...
let evening,hemi,roadsideGroup,signData,signGroup,signalSystem,furnitureGroup,weather;
...
async function init(){
 ...
 get('./data/tours.json').then(({tours})=>{const box=$('tourList');if(!box)return;for(const t of tours){const b=document.createElement('button');b.textContent=t.name+' · '+t.stops.length+' stops';b.title=t.blurb;b.onclick=()=>{tour?.stop();driving.startTour(t);};box.append(b);}}).catch(()=>{});
...
window.__drive=driving; ambient=new AmbientLife(scene,driving.state.network,surfaceAt);driving.cruise.traffic=ambient;driving.cruise.stops=assetData.assets.filter(a=>a.kind==='stop');tour=new CoastalTour(camera,controls,surfaceAt,()=>{$('tour').textContent='Coastal flyover';});
 canopyData=await get('./data/tree-survey.json');canopyGroup=treeSurvey(canopyData,surfaceAt);scene.add(canopyGroup);
 const canopyAreas=await get('./data/lidar-trees.json').catch(()=>get('./data/canopy-scenery.json'));sceneryTrees=await treeScenery(canopyData,surfaceAt,driving.state.network,canopyAreas);scene.add(sceneryTrees);$('treeSummary').textContent=sceneryTrees.userData.count.toLocaleString()+' trees measured by the 2021 USGS LiDAR: crown position, height and radius per tree ('+sceneryTrees.userData.areaCount.toLocaleString()+' from the point cloud, '+sceneryTrees.userData.screened+' screened from aerials). Species estimated.';
...
function animate(time){const dt=lastTime?Math.min((time-lastTime)/1000,.1):.016;lastTime=time;
 ...
 if(frame%6===0)furnitureGroup?.userData.lights?.update(time/1000);weather?.update(dt,camera);
