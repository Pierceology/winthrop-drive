...
export class PhotoFronts {
 constructor({scene,world,neighborhoods,photos,network,anisotropy=4,radius=240,capacity=70,concurrency=3}){
  ...
  for(const [id,p] of Object.entries(photos)){const b=byId.get(id);if(!b||b.residence||!p.w||!p.h)continue;this.entries.push({id,pid:p.pid,w:p.w,h:p.h,b,record:neighborhoods.buildings[id],state:'idle',mesh:null,distance:Infinity});}
 }
 ...
 build(e,texture){
  ...
  const natural=w.len*e.h/e.w,H=Math.max(eave,Math.min(natural,ridge)),qa=w.len/H,pa=e.w/e.h;
  let u0=0,u1=1,v0=0,v1=1;if(qa>pa){const k=pa/qa;v0=.5-k/2;v1=.5+k/2;}else{const k=qa/pa;u0=.5-k/2;u1=.5+k/2;}
  ...
  const m=new THREE.Mesh(g,new THREE.MeshLambertMaterial({map:texture,side:THREE.DoubleSide,transparent:true,alphaTest:.02}));m.receiveShadow=true;m.userData.photoFront=e.id;return m;
 }
 update(x,z){
 ...
 async load(e){
  ...
  try{
   const image=new Image();image.decoding='async';image.src='./assets/assessor/'+e.pid+'.jpg';await image.decode();
   if(this.wanted.has(e.id)){
    const canvas=document.createElement('canvas');canvas.width=image.width;canvas.height=image.height;const ctx=canvas.getContext('2d');ctx.drawImage(image,0,0);const fade=ctx.createLinearGradient(0,0,0,image.height*.14);fade.addColorStop(0,'rgba(0,0,0,1)');fade.addColorStop(1,'rgba(0,0,0,0)');ctx.globalCompositeOperation='destination-out';ctx.fillStyle=fade;ctx.fillRect(0,0,image.width,image.height*.14);
