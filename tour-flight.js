...
export class TourFlight{
 ...
 go(i){if(!this.tour)return;const n=this.tour.stops.length;if(!n){this.stop();return;}this.index=((i%n)+n)%n;const st=this.tour.stops[this.index],prev=this.tour.stops[(this.index-1+n)%n],y=this.heightAt(st.x,st.z);
  ...
  const cap=document.getElementById('tourCaption'),kick=document.getElementById('tourKicker');if(cap)cap.textContent=st.name;if(kick)kick.textContent=this.tour.name.toUpperCase()+' · '+(this.index+1)+' of '+n;const card=document.getElementById('tourCard');if(card){const img=card.querySelector('img'),line=card.querySelector('.line'),addr=card.querySelector('.addr');if(st.photo){img.src=st.photo;img.hidden=false;}else{img.removeAttribute('src');img.hidden=true;}line.textContent=st.line||'';addr.textContent=st.address||'';card.hidden=!(st.photo||st.line);}}
